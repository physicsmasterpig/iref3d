//! Integration test: multi-cusp sequential refined Dehn filling.

use iref3d_core::{
    census::Census,
    refined_dehn::multi_cusp::{compute_multi_cusp_filled_refined_index, MultiCuspFillSpec},
...
fn multi_cusp_matches_v05() {
    ...
    for case in &g.cases {
        ...
        for fill in &case.fills {
            let ctx = format!(
                "{}: ({}/{},{}/{})",
                case.name, fill.P0, fill.Q0, fill.P1, fill.Q1
            );
            println!(
                "Testing {ctx} hj_ks={:?} num_cusp_eta={}",
                fill.hj_ks, fill.num_cusp_eta
            );
            ...
            let got = compute_multi_cusp_filled_refined_index(
                &state,
                case.num_hard,
                &specs,
                fill.qq_order,
            ...
            assert_eq!(got.num_cusp_eta, fill.num_cusp_eta, "{ctx}: num_cusp_eta");
